...
台股權證標的篩選腳本 V8
V8 修正：
  1. 啟動時立即檢查 FINMIND_TOKEN，若未設定直接印出醒目警告
  2. stocks.json 加入 is_demo 旗標，前端偵測到 DEMO 資料時顯示警告
  3. 若 FinMind 完全拿不到資料（token 失效/超限），寫入 is_demo:true 的佔位資料並明確告知
  4. 其餘邏輯同 V7，評分算法不變
"""
    ...
    for i in range(retries):
        try:
            ...
            if r.status_code == 402:
                print("  ! FinMind 超出使用上限（免費 300/hr），請設定 FINMIND_TOKEN 或稍後再試")
                return []
            if r.status_code == 401:
                print("  ! FinMind Token 無效或已過期，請至 finmindtrade.com 重新取得")
                return []
            r.raise_for_status()
            d = r.json()
            if d.get("status") == 200:
...
def main():
    ...
    print(f"\n[{now.strftime('%H:%M:%S')} 台灣時間] MOUP V8 開始抓取 {today} 資料...")
    print("=" * 60)

    # V8：啟動時立即檢查 token
    if FINMIND_TOKEN:
        print(f"  ✅ FINMIND_TOKEN 已設定（長度 {len(FINMIND_TOKEN)} 字）")
    else:
        print("  ⚠️  警告：FINMIND_TOKEN 未設定！")
        print("     → 匿名模式上限 300 req/hr，掃描 1800+ 支個股會超限")
        print("     → 請至 finmindtrade.com 免費註冊並在 GitHub Secrets 加入 FINMIND_TOKEN")
    ...
    if not stock_info:
        print("  ! 無法取得股票清單，中止（請確認網路或 FINMIND_TOKEN）")
        # V8：失敗時不留 DEMO 資料，改寫 is_demo:true 明確標記
        if os.path.exists(OUTPUT_PATH):
            with open(OUTPUT_PATH, encoding="utf-8") as f:
                existing = json.load(f)
            if existing.get("is_demo"):
                print("  → 現有資料已是 DEMO，不覆蓋")
            else:
                print("  → 保留現有 stocks.json 不覆蓋")
        return
    ...
    # ③ 候選股
    candidate_sids = [s for s in warrants.keys() if s in stock_info]
    print(f"\n  → 有認購權證的候選股：{len(candidate_sids)} 支")
    ...
    for idx, sid in enumerate(candidate_sids):
        ...
        market = info["market"]
        if (idx + 1) % 20 == 0:
            print(f"  ... 已處理 {idx+1}/{len(candidate_sids)} 支，共 {total_req} 次 API 請求")

        price_hist = fetch_price_one(sid, start_date, end_date)
        total_req += 1
        time.sleep(0.2)
        if not price_hist: continue

        today_price = price_hist[-1]
        if today_price["date"] < past[0]: continue
        ...
        score, reasons, warnings, chg_pct = calc_score(sid, today_price, hist, inst_today, margin)
        if sid in warrants:
        ...
        if total_req >= 280:
            print(f"  ! 接近 API 限制（{total_req} req），停止掃描")
            break
    ...
    if len(top10) == 0 and os.path.exists(OUTPUT_PATH):
        with open(OUTPUT_PATH, encoding="utf-8") as f:
            old = json.load(f)
        if old.get("is_demo"):
            print("\n  ⚠ 本次結果為 0 支，且現有為 DEMO 資料 — 保留 DEMO 不覆蓋")
            print("  → 請確認：1) FINMIND_TOKEN 已設定  2) 盤後(14:00後)執行  3) API 未超限")
        else:
            print("\n  ⚠ V8保護：本次精選結果為 0 支，保留上次真實 stocks.json，不覆蓋")
        return

    output = {
        "updated_at":       tw_now().strftime("%Y/%m/%d %H:%M"),
        "trade_date":       today8,
        "is_demo":          False,  # V8：真實資料旗標
