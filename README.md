...
- **V41（目前版本）**：移除「有發行但抓不到明細」的多餘藍色說明區塊。
  這個標的展開後，若權證有發行但抓不到明細，原本會多跳出一個藍框
  （「此標的名下有發行認購權證，但目前抓不到明細…」+ 教你去證券 APP 查），
  跟區塊標題旁本來就有的「✅ 有發行」小標籤重複，拿掉後只保留標籤本身，
  版面更乾淨。
- V40：隔夜精選速覽移除卡片內重複提示。每張卡片原本
  都會印一次「📱 請至證券 APP 查看今日即時走勢」，跟頁籤最上方已有的
  提醒完全重複，拿掉後版面更乾淨、資訊不重複。
- V39：footer 補回版次號碼。加回被移除的版次文字「V39」，
  放在「最後更新」時間之前，樣式與旁邊其他 footer-meta 文字一致（低調小字，
  不特別強調），避免完全找不到目前版本。
- V38：LOGO 改為置頂置中、融合背景樣式。① 去除浮動固定
